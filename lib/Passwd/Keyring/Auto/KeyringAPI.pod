...
password of the same user in the same domain, but with different
...
C<password_domain> (or C<group> given in constructor) leads to
separate password.
...
Actual key for the password should be built from C<group> given in C<new>, C<password_domain>
...
name, group name, username, password and password domain syntax.  This
means escaping or replacing possibly illegal characters (for example,
if some implementation dislikes C</> in password domain, it should
escape it somehow).
